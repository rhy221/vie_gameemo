...
# P0.0-FIX — Wiring extract_features + ràng buộc input pose (sau khảo sát P0.0)

> Khảo sát xác nhận `temporal_pool="mean"` mù chuyển động (đúng nghi vấn). Hạ tầng branch đã sẵn; hở duy nhất là `scripts/extract_features.py` khởi tạo `ContextEncoder` trực tiếp. Khi sửa, kẹp 5 ràng buộc dưới để lần chỉnh này không gài bất đối xứng vào ablation hay nhiễu cross-streamer. Bối cảnh: dữ liệu là **webcam overlay của streamer** (đã tách khỏi gameplay để tránh nhiễu game); crop **tùy streamer** nhưng **thường thấy cả thân trên**.

## P0.0-FIX.1 — Dùng factory + CÙNG input cho hai nhánh (`scripts/extract_features.py`)
Thay khởi tạo cứng `ContextEncoder(...)` bằng `get_context_encoder(cfg)`, rẽ nhánh phần gọi encode. **Cả hai nhánh nhận cùng webcam crop** — pose **KHÔNG** chạy full frame. Lý do kép: (a) full frame có gameplay → pose detector bắt nhầm nhân vật game/HUD/người khác thành keypoint rác; (b) nếu vit dùng crop còn pose dùng full frame, ablation P0.3 sẽ lẫn "pose vs vit" với "crop vs full frame" → không diễn giải được. Cố định cùng nguồn frame, chỉ khác cách mã hóa.

## P0.0-FIX.2 — Pose chạy MỘT lần, cache dùng chung (`context_pose.py` + `cue_extractor.py`)
Chạy pose backend **một lần** → cache `(T,K,4)`. *Cả* `PoseContextEncoder` (feature) *lẫn* `MotionCueExtractor.precompute` (cue) đọc từ **cùng cache** — cùng backend, cùng keypoint set, cùng config. Tránh hai lần chạy pose lệch tham số (đúng họ "hai phép tính song song dễ lệch" của FIX 7.1/8.2, ở tầng input). Thống nhất một tên method cho pose encoder.

## P0.0-FIX.3 — Keypoint set xử lý "có mặt một phần", không hardcode (`context_pose.py`)
Crop tùy streamer ⇒ tay ra/vào khung. Lấy **đủ thân trên + tay + đầu**, để **confidence-mask (FIX 6.3/8.1) tự xử lý phần ngoài khung** (tay ngoài khung → conf thấp → masked → không tín hiệu giả; tay trong khung → đóng góp). KHÔNG cấu hình cứng "dùng tay" (crop chật → toàn gap) hay "bỏ tay" (crop rộng → phí tín hiệu).

## P0.0-FIX.4 — Neo impact/cue vào keypoint LUÔN-hiện-diện (`cue_extractor.py`)
Định nghĩa "impact"/motion cue theo **gia tốc thân/vai/đầu** (luôn trong khung ở hầu hết facecam), KHÔNG theo cổ tay. Nếu neo vào cổ tay, streamer crop chật sẽ gần như không bao giờ có cue "đập bàn" (tay ngoài khung) → cue **không nhất quán giữa các streamer** dù cùng hành vi. Tay, khi có trong khung, là **bằng chứng bổ sung**, không phải điều kiện. (Lưu ý: một phần tín hiệu "đập bàn" vốn thuộc về **audio** — tiếng "rầm" — không phải context.)

## P0.0-FIX.5 — Chuẩn hóa tọa độ theo kích thước người trong khung (`context_pose.py`)
Khung mỗi streamer một cỡ ⇒ tọa độ pixel tuyệt đối không so sánh được. Chuẩn hóa keypoint theo **kích thước người trong khung** (khoảng cách vai, hoặc bbox thân) **trước khi** tính kinematic. Nếu không, "motion energy" sẽ lẫn "facecam to/nhỏ" với "streamer động nhiều/ít" — nhiễu âm thầm làm hỏng cả feature lẫn cue.

## Lưu ý cho P0.3 (không phải việc của lần sửa này)
- **Sampling pose tách khỏi 16-frame-uniform của ViT:** xung "đập bàn" chỉ vài frame, lấy đều 16 frame dễ bỏ lỡ. Pose nên sample dày hơn hoặc theo chuyển động; để thành tham số riêng theo nhánh, calibrate ở P0.3.
- **Kiểm phân bố confidence thực tế:** webcam overlay thường nhỏ/mờ → confidence nền thấp, mask kích hoạt thường hơn dự kiến. Phân biệt "conf thấp do ảnh mờ" (diện rộng, không phải tín hiệu) với "conf thấp do chuyển động mạnh" (P0.2 — tín hiệu arousal). Nới ngưỡng mask nếu nền thấp diện rộng.

---

# P0.0-fix — Hoàn thiện branch selection + nhất quán input hai nhánh

> Từ khảo sát P0.0: hạ tầng branch đã sẵn (factory, `PoseContextEncoder`, config gate). Hở duy nhất về *hạ tầng* là `scripts/extract_features.py` còn khởi tạo `ContextEncoder` cứng. Nhưng khi sửa, phải kẹp các ràng buộc dưới để lần chỉnh này không gài bất đối xứng vào phép so sánh ablation. Bối cảnh dữ liệu: **video stream game, chỉ lấy webcam overlay của streamer** (tránh nhiễu gameplay); crop **tùy streamer** nhưng **thường thấy cả thân trên**.

## P0.0-fix.1 — extract_features.py dùng factory, hai nhánh CÙNG input
- Thay khởi tạo cứng `ContextEncoder(...)` bằng `get_context_encoder(cfg)`, rẽ nhánh phần gọi encode.
- **Cả hai nhánh nhận CÙNG webcam crop.** Pose **không** chạy trên full frame. Lý do kép: (a) ablation phải so "pose vs vit", không lẫn "crop vs full frame"; (b) pose trên full frame sẽ bắt nhầm người/nhân vật trong gameplay → keypoint rác. Crop overlay loại nguyên lớp nhiễu đó.

## P0.0-fix.2 — Pose chạy MỘT lần, encoder + cue đọc chung cache
Chạy pose backend **một lần** → cache `(T, K, 4)` (x,y,z,conf). *Cả* `PoseContextEncoder` *lẫn* `MotionCueExtractor.precompute` đọc từ **cùng cache** (cùng backend, cùng keypoint set, cùng sampling). Cấm hai lần chạy pose lệch tham số — đó là biến thể tầng-khác của bug "hai phép tính song song" (FIX 7.1/8.2). Thống nhất một tên method cho pose encoder.

## P0.0-fix.3 — Keypoint set: thân trên + tay + đầu, để confidence-mask xử lý phần ngoài khung
Crop tùy streamer ⇒ **không hardcode "có/không tay".** Lấy đủ tập thân trên + hai tay + đầu; dựa vào confidence-mask (FIX 6.3/8.1) tự xử lý: tay ngoài khung → conf thấp → mask → không sinh tín hiệu giả; tay trong khung → đóng góp. (Hệ thống mask đã xây chính là thứ làm "crop tùy streamer" trở nên vô hại.)

## P0.0-fix.4 — Neo định nghĩa impact/cue vào keypoint LUÔN-hiện-diện (thân/vai/đầu)
Vì crop chật hay đẩy **cổ tay ra ngoài khung**, KHÔNG định nghĩa "đập bàn" theo gia tốc cổ tay (sẽ không bao giờ kích hoạt ở crop chật → cue **không nhất quán giữa streamer**: cùng hành vi, crop rộng có cue, crop chật không). Neo impact/cue vào **giật/lao của thân + đầu** (gần như mọi facecam đều thấy); cổ tay, nếu trong khung, là *bằng chứng bổ sung* chứ không phải điều kiện. Mục tiêu: cue đồng đều cross-streamer.

## P0.0-fix.5 — Chuẩn hóa tọa độ theo kích thước người trong khung
Khung mỗi streamer một cỡ ⇒ tọa độ pixel tuyệt đối không so sánh được. Trước khi tính kinematic, **chuẩn hóa keypoint theo kích thước người** (khoảng cách vai, hoặc bbox thân). Nếu không, "motion energy" sẽ lẫn với "facecam to/nhỏ" thay vì "streamer động nhiều/ít" — nhiễu âm thầm làm hỏng cả feature lẫn cue. Áp cùng chuẩn hóa cho cả encoder lẫn cue (đọc chung cache ở fix.2 nên chỉ chuẩn hóa một chỗ).

## P0.0-fix.6 — Sampling pose tách riêng khỏi ViT, calibrate ở P0.3
ViT `_uniform_sample` 16 frame ổn cho cảnh tĩnh, nhưng "đập bàn" là xung vài frame → lấy đều 16 frame dễ **bỏ lỡ khung xung**. Tách tham số sampling của pose khỏi ViT (sample dày hơn, hoặc theo motion). Để giá trị cụ thể calibrate ở P0.3. Ngoài ra, webcam overlay thường nhỏ/mờ ⇒ confidence nền thấp diện rộng: ở P0.3 kiểm phân bố confidence, phân biệt "conf thấp do ảnh mờ" với "conf thấp do chuyển động mạnh" — chỉ cái sau là tín hiệu arousal (P0.2); nới ngưỡng mask nếu cần.
